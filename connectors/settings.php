		...
		$rules = array(
			'stream' => array(
				...
				'url'          => function( $rule, $record ) {
					$option_key = wp_get_stream_meta( $record->ID, 'option_key', true );
					$url_tab    = null;
			...
			if ( ! empty( $applicable_rules ) ) {
				...
				if ( ! empty( $found_submenus ) ) {
					...
					if ( current_user_can( $capability ) ) {
						...
						$field_name = wp_get_stream_meta( $record->ID, 'option_key', true );

						if ( '' === $field_name ) {
							$field_name = wp_get_stream_meta( $record->ID, 'option', true );
						}
