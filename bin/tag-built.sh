...
tag=$(cat build/stream.php | grep 'Version:' | sed 's/.*: //' | sed 's/-[0-9]\{8\}T[0-9]\{6\}Z-[a-f0-9]*$//')
if [[ -z "$tag" ]]; then
    echo "Error: Unable to determine tag from build/stream.php."
    exit 1
fi
...
echo "See https://github.com/xwp/wp-stream/releases/tag/$built_tag"
echo "For https://github.com/xwp/wp-stream/releases/tag/$tag"
