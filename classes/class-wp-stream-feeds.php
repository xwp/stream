...
class WP_Stream_Feeds {
	...
	const USER_FEED_OPTION       = 'stream_user_feed_key';
	...
	public static function generate_user_feed_key() {
		...
		if ( $user_id ) {
			...
			update_user_meta( $user_id, self::USER_FEED_OPTION, $feed_key );
	...
	public static function save_user_feed_key( $user ) {
		...
		if ( ! $generate_key && get_user_meta( $user->ID, self::USER_FEED_OPTION, true ) ) {
			return;
		}
		...
		update_user_meta( $user->ID, self::USER_FEED_OPTION, $feed_key );
	}
	...
	public static function user_feed_key( $user ) {
		...
		$key  = get_user_meta( $user->ID, self::USER_FEED_OPTION, true );
		...
		<table class="form-table">
			<tr>
				<th><label for="<?php echo esc_attr( self::USER_FEED_OPTION ) ?>"><?php esc_html_e( 'Stream Feeds Key', 'stream' ) ?></label></th>
				<td>
					<p class="wp-stream-feeds-key">
						<?php wp_nonce_field( 'wp_stream_generate_key', 'wp_stream_generate_key_nonce' ) ?>
						<input type="text" name="<?php echo esc_attr( self::USER_FEED_OPTION ) ?>" id="<?php echo esc_attr( self::USER_FEED_OPTION ) ?>" class="regular-text code" value="<?php echo esc_attr( $key ) ?>" readonly>
						<small><a href="<?php echo esc_url( add_query_arg( array( self::GENERATE_KEY_QUERY_VAR => true, 'wp_stream_nonce' => $nonce ) ) ) ?>" id="<?php echo esc_attr( self::USER_FEED_OPTION ) ?>_generate"><?php esc_html_e( 'Generate new key', 'stream' ) ?></a></small>
	...
	public static function feed_template() {
		...
		$args = array(
			'meta_key'   => self::USER_FEED_OPTION,
