...
class Unread {
	...
	public function __construct( $plugin ) {
		...
		add_filter( 'wp_stream_admin_menu_title', array( $this, 'admin_menu_title' ) );

		// Catch list table results
		add_filter( 'wp_stream_query_results', array( $this, 'mark_as_read' ), 10, 3 );

		// Add user option for enabling unread counts
		add_action( 'show_user_profile', array( $this, 'render_user_option' ) );
		add_action( 'edit_user_profile', array( $this, 'render_user_option' ) );

		// Save unread counts user option
		add_action( 'personal_options_update', array( $this, 'save_user_option' ) );
		add_action( 'edit_user_profile_update', array( $this, 'save_user_option' ) );
	 ...
	 * @return bool
	 */
	public function unread_enabled_for_user( $user_id = 0 ) {
		$user_id = empty( $user_id ) ? get_current_user_id() : $user_id;
		$enabled = wp_stream_get_user_meta( $user_id, self::UNREAD_COUNT_OPTION_KEY );
		$enabled = ( ! $this->plugin->is_vip() || 'off' !== $enabled );
	...
	public function get_unread_count() {
		if ( ! $this->unread_enabled_for_user() ) {
			return false;
		}
	 ...
	 * @return string
	 */
	public function admin_menu_title( $menu_title ) {
		$unread_count = $this->get_unread_count();

		if ( $this->unread_enabled_for_user() && ! empty( $unread_count ) ) {
	 ...
	 * @return array
	 */
	public function mark_as_read( $results, $query, $fields ) {
		unset( $fields );
		...
		$is_list_table = ( isset( $screen->id ) && sprintf( 'toplevel_page_%s', $this->plugin->admin->records_page_slug ) === $screen->id );
		...
		if ( $is_list_table && $is_first_page && $is_date_desc ) {
			...
			if ( $this->unread_enabled_for_user() && isset( $results[0]->created ) ) {
	 ...
	 * @param \WP_User $user
	 */
	public function render_user_option( $user ) {
		if ( ! array_intersect( $user->roles, $this->plugin->settings->options['general_role_access'] ) ) {
			return;
		}

		$unread_enabled = $this->unread_enabled_for_user();
		?>
	 ...
	 * @param $user_id
	 */
	public function save_user_option( $user_id ) {
	 ...
	 * @param int $user_id
	 */
	public function delete_transient( $user_id ) {
		$cache_key = sprintf( '%s_%d', self::UNREAD_COUNT_OPTION_KEY, $user_id );
