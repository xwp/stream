...
class WP_Stream_Extensions {
	...
	const EXTENSIONS_OPTION = 'wp_stream_extensions_';
	const MEMBER_OPTION     = 'wp_stream_member';
	...
	function get_extension_data( $force = false ) {
		if ( $force || false === ( $api_transient = get_transient( self::EXTENSIONS_OPTION ) ) ) {
			$api_transient = $this->get_extension_api();

			if ( $api_transient && ! empty( $api_transient ) ) {
				set_transient( self::EXTENSIONS_OPTION, $this->get_extension_api(), MINUTE_IN_SECONDS * 60 * 6 );
			}
