...
class Alert_Type_Slack extends Alert_Type {
  ...
  public function alert( $record_id, $recordarr, $alert ) {
    ...
    $fields = array(
      array('title' => 'IP Address', 'value' => $recordarr['ip'], 'short' => true),
      array('title' => 'Connector', 'value' => $recordarr['connector'], 'short' => true),
      array('title' => 'Context', 'value' => $context, 'short' => true),
      array('title' => 'Action', 'value' => $action, 'short' => true),
    );
    ...
    if ( is_object( $post ) && ! empty( $post ) ) {
      ...
      array_push($fields, array('title' => "Edit ".$post_type->labels->singular_name, 'value' => "<$edit_post_link>", 'short' => false));
    }

    $edit_alert_link = admin_url( 'edit.php?post_type=wp_stream_alerts#post-' . $alert->ID );
    array_push($fields, array('title' => 'Edit Alert', 'value' => "<$edit_alert_link>", 'short' => false));
    ...
    if(!empty($options['icon'])) {
      ...
      } else if(substr($options['icon'], 0, 4) === "http") {
        $data['icon_url'] = $options['icon'];
      }
    }
    wp_remote_post($options['webhook'], array('body' => json_encode($data), 'headers' => array('Content-Type' => 'application/json')));
  }
  /**
  ...
  public function display_fields( $alert ) {
    ...
    echo '</span>';
    echo '<span class="input-text-wrap">The webhook URL</span>';
    echo '</label>';
    ...
    echo '</span>';
    echo '<span class="input-text-wrap">The channel to send to (optional)</span>';
    echo '</label>';
    ...
    echo '</span>';
    echo '<span class="input-text-wrap">The username to send as (optional)</span>';
    echo '</label>';
    ...
    echo '</span>';
    echo '<span class="input-text-wrap">The URL or emoji (with colons!) to use as the icon (optional)</span>';
    echo '</label>';
  }
  /**
  ...
  public function save_fields( $alert ) {
    ...
    if ( isset( $_POST['wp_stream_slack_webhook'] ) ) {
      $alert->alert_meta['webhook'] = sanitize_text_field( wp_unslash( $_POST['wp_stream_slack_webhook'] ) );
    }
    if ( isset( $_POST['wp_stream_slack_channel'] ) ) {
      $alert->alert_meta['channel'] = sanitize_text_field( wp_unslash( $_POST['wp_stream_slack_channel'] ) );
    }
    if ( isset( $_POST['wp_stream_slack_username'] ) ) {
      $alert->alert_meta['username'] = sanitize_text_field( wp_unslash( $_POST['wp_stream_slack_username'] ) );
    }
    if ( isset( $_POST['wp_stream_slack_icon'] ) ) {
      $alert->alert_meta['icon'] = sanitize_text_field( wp_unslash( $_POST['wp_stream_slack_icon'] ) );
    }
  }
  /**
