...
class Connector_Jetpack_Test extends WP_StreamTestCase {
	...
	public function test_callback_jetpack_log_entry_activate() {
		$this->mock->expects( $this->once() )
			->method( 'log' )
			->with(
				$this->equalTo( 'Comments module activated' ),
				$this->equalTo( array( 'module_slug' => 'comments' ) ),
				$this->equalTo( null ),
	...
	public function test_callback_jetpack_log_entry_authorize() {
		...
		$this->mock->expects( $this->once() )
			->method( 'log' )
			->with(
				$this->equalTo( 'testuser\'s account linked to Jetpack' ),
				$this->equalTo(
					array(
						'user_id'    => $user_id,
	...
	public function test_callback_jetpack_log_entry_register() {
		$this->mock->expects( $this->once() )
			->method( 'log' )
			->with(
				$this->equalTo( 'Site connected to Jetpack' ),
				$this->equalTo( array() ),
				$this->equalTo( null ),
				$this->equalTo( 'blogs' ),
	...
	public function test_check_add_sharing_options() {
	...
	public function test_check_update_sharing_options() {
		$admin_id = self::factory()->user->create( array( 'role' => 'administrator' ) );
		wp_set_current_user( $admin_id );

		// Seed sharing-options so the next REST call performs an update.
		$request = new \WP_REST_Request( 'POST', "{$this->namespaced_route}/settings" );
		$request->set_body_params(
			array(
				'carousel'                  => true,
	...
	public function test_track_post_by_email_enable() {
	...
	public function test_track_post_by_email_disable() {
		$admin_id = self::factory()->user->create(
			array(
				'role'         => 'administrator',
				'display_name' => 'TestGuy',
	...
	public function test_track_post_by_email_regenerate() {
		$admin_id = self::factory()->user->create(
			array(
				'role'         => 'administrator',
				'display_name' => 'TestGuy',
	...
	public function test_check_jetpack_options_logs_only_the_changed_field() {
		...
		$connector->expects( $this->once() )
			...
			->with(
				$this->anything(),
				$this->callback( array( self::class, 'assert_jetpack_options_changed_field_only' ) )
			);
	...
	/**
	 * Assert VideoPress change is logged without credential leakage.
	 *
	 * @param mixed $logged Log context argument.
	 * @return bool
	 */
	public static function assert_jetpack_options_changed_field_only( $logged ): bool {
		if ( ! is_array( $logged ) ) {
			return false;
		}
		if ( 'edit_posts' !== ( $logged['old_value'] ?? null ) || 'read' !== ( $logged['value'] ?? null ) ) {
			return false;
		}
		$serialized = maybe_serialize( $logged );
		return false === strpos( $serialized, 'user.token.one' )
			&& false === strpos( $serialized, 'facebook.access.token' );
	}
	...
	public function test_check_jetpack_options_withholds_publicize_connection_payload() {
		...
		$connector->expects( $this->atLeastOnce() )
			...
			->with(
				$this->anything(),
				$this->callback( array( self::class, 'assert_publicize_connection_payload_redacted' ) )
			);
	...
	/**
	 * Assert Publicize connection payloads are redacted.
	 *
	 * @param mixed $logged Log context argument.
	 * @return bool
	 */
	public static function assert_publicize_connection_payload_redacted( $logged ): bool {
		if ( ! is_array( $logged ) ) {
			return false;
		}
		...
		$serialized = maybe_serialize( $logged );
		return false === strpos( $serialized, 'facebook.access.token' )
			&& false === strpos( $serialized, 'facebook.token.secret' );
	}
}
